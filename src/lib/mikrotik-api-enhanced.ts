...
export class MikroTikAPI {
  ...
  constructor(connection: MikroTikConnection, apiUrl: string = (import.meta.env.VITE_BACKEND_URL || 'http://localhost:3001/api/mikrotik')) {
  ...
  // Create hotspot user (voucher) via backend API with improved error handling
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          throw new Error(`Rate limit exceeded. Please wait before creating more vouchers. Server response: ${errorText.substring(0, 100)}...`);
        } else {
          throw new Error(`HTTP ${response.status}: ${response.statusText}. Server response: ${errorText.substring(0, 100)}...`);
        }
      }
  ...
  // Create hotspot users in batch via backend API with retry logic
  async createHotspotUsersBatch(users: HotspotUser[], maxRetries: number = 3): Promise<any> {
    try {
      if (users.length > 1000) {
        throw new Error('Maximum 1000 users allowed in batch');
      }
